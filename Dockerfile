...
# /home/ash/ref: H37Rv, the gold standard TB reference
...
# /home/ash/example_tree: "known lineage SRA tuberculosis tree", FOR QUICK TESTS ONLY!
...
# Install UShER
# * This needs to happen before conda throws up on PATH
# * At some point I ran RUN git checkout $(git describe --tags $(git rev-list --tags --max-count=1)) but I can't get
#   the actual 0.6.6 tagged commit to compile, so we're just grabbing main!
...
# Conda stuff
# Conda needs to be on the path or else BTE won't install properly
...
WORKDIR /home/BTE
RUN /opt/conda/bin/mamba env update -n base --file bte.yml && \
    /opt/conda/bin/mamba clean --all -y
RUN export CONDA_PREFIX=/opt/conda && \
    /opt/conda/bin/python3 setup.py build_ext && \
...
# /home/ash/scripts: Various Tree Nine scripts
WORKDIR /home/ash
RUN mkdir /scripts/
