# Tree Nine
Place samples on an existing phylogenetic tree using [UShER](https://www.nature.com/articles/s41588-021-00862-7)'s `usher-sampled` task with a bit of help from [SRANWRP](https://www.github.com/aofarrel/SRANWRP), followed by conversion of that tree to Taxonium, Newick, and Nextstrain formats. If you enable clustering, samples' SNP distance is calculated from branch length and output as a distance matrix, and samples will be placed into clusters based on the distance. Clustering can be done on a subset of samples (recommended) or the entire tree.
