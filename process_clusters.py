...
def main():
    ...
    all_latest_samples = all_latest_samples.filter(all_latest_samples.select(pl.col("latest_cluster_id").is_not_null()))
    all_persistent_samples = pl.read_csv(args.persistentids, separator="\t", schema_overrides={"cluster_id": pl.Utf8})
    all_persistent_samples = all_persistent_samples.filter(all_persistent_samples.select(pl.col("latest_cluster_id").is_not_null()))
    ...
    persistent_clusters_meta = persistent_clusters_meta.filter(persistent_clusters_meta.select(pl.col("latest_cluster_id").is_not_null()))
