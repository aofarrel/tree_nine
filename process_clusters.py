...
def main():
    ...
    all_latest_samples = all_latest_samples.filter(~all_latest_samples.row(0).is_null().all())
    all_persistent_samples = pl.read_csv(args.persistentids, separator="\t", schema_overrides={"cluster_id": pl.Utf8})
    all_persistent_samples = all_persistent_samples.filter(~all_persistent_samples.row(0).is_null().all())
    ...
    persistent_clusters_meta = persistent_clusters_meta.filter(~persistent_clusters_meta.row(0).is_null().all())
